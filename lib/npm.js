...
const BINS = ['cnpm', 'yarn', 'npm']; function cached (fn) { let _cache = [];
...
module.exports = {
  ...
  version (pkg) {
    return this.command().then(command => {
      ...
      if (command !== 'yarn') {
        return execa(command, ['info', pkg, 'version'], opts).then(({ stdout }) => stdout);
      } else {
        return execa(command, ['info', pkg, 'version'], opts).then(({ stdout }) => {
          return stdout.split('\n')[1];
        });
      }
    });
